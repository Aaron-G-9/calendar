...
import OverlayTrigger from 'react-bootstrap/lib/OverlayTrigger'; //react-bootstrap
import Popover from 'react-bootstrap/lib/Popover'; //react-bootstrap

//Variables for more easily accessing props. Perhaps this is a more proper way
//to make components stateless...
var currentMonth;
var currentYear;
var currentDay;
var currentWeek;
...
class HourBox extends React.Component {
    //Uses the method passed through from the parent to grab the state component
    getFirstDayofMonth() {
    ...
    render() {
        if (DateHelper.getWeekArray(currentMonth, currentYear, currentWeek)[this.props.dayOfWeek] != 0) {
            ...
            return (
                <div className="greyedHourBox"></div>
            )
        }
...
class HourContent extends React.Component {
    render() {
        ...
        for (var i = 0; i < this.props.courseObject.myCourses.length; i++) {
            ...
            if (content.getStartMonth() <= this.props.getDateSelection().selectedMonth && content.getEndMonth() >= this.props.getDateSelection().selectedMonth && this.props.hourBoxNumber == content.getWeekViewPosition() && content.getMeetDays()[day.getDay()] == true) {
                //console.log(content.getWeekViewPosition() + " " + this.props.hourBoxNumber);
                return (
                    <div className="filledHourBoxContent">
                        <OverlayTrigger trigger="click" rootClose placement="bottom" overlay={< Popover title = {
                            content.getTitle()
                        } > <PopoverContent location={content.getBuilding()} time={content.getStartTimeInfo("string") + " - " + content.getEndTimeInfo("string")}/> < /Popover>}>
